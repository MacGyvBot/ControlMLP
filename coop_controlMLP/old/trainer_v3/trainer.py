...
URDF_PATH = r"/home/whatdojdo/Desktop/coop_controlMLP/m0609.urdf" # 실제 경로로 수정
EE_LINK_NAME = "link_6"  # 엔드 이펙터 링크 이름 (예시)
Z_OFFSET = 0.22   #in meters 그리퍼가 link_6로부터 가진 오프셋

MAX_DELTA_Q = 0.05 # 1스텝당 관절 최대 변화량 (Radian)

DIST_ARRIVED_THRES = 0.01 #목표 좌표와 거리가 이정도 차이 나는건 그냥 도착으로 간주하겟다 (1cm)
...
# 초기부터 진행시 None
LOAD_WEIGHTS_PATH = r"/home/whatdojdo/Desktop/coop_controlMLP/trainer_v3/complete_train/trained_mlp_weights_20k_epochs.pth"

#DEVICE = torch.device("cpu")
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
BATCH_SIZE = 4096
MAX_STEPS = 200          # 가상 롤아웃 스텝 수 (N)
LR = 3e-3  # 기존 1e-3에서 3배 증가
EPOCHS = 10000
...
#loss range 계산
LOSS_RANGE = (1.0 + 1.0) + (SMOOTH_LOSS_SCALE + SINGULAR_LOSS_SCALE + STEP_LOSS_SCALE) #메인로스 + 서브로스
...
def get_kinematics_of_GRIPPER(q):
    ...
    g_curr = T_tcp[:, :3, 2]    # 로컬 Z축이 향하는 방향 (접근 벡터)
    g_up_curr = T_tcp[:, :3, 0] # 로컬 X축이 향하는 방향 (Up 벡터)
...
for epoch in range(EPOCHS):
    ...
    for step in range(MAX_STEPS):
        ...
        cos_A = torch.sum(g_curr * A_target, dim=1) #목표와의 벡터 오차
        cos_A_up = torch.sum(g_up_curr * A_up_target, dim=1) #목표와의 벡터 오차
        ...
        # step loss
        loss_step += (dist / MAX_WORKSPACE_DIST).mean()
    ...
    cos_A = 1.0 - torch.sum(g_curr * A_target, dim=1)       # (Batch,)
    cos_A_up = 1.0 - torch.sum(g_up_curr * A_up_target, dim=1) 
    loss_cosine = (cos_A + cos_A_up).mean() / 4.0 # 최대 4.0이므로 정규화 [LOSS COSINE]
    ...
    # 로그 출력
    if epoch % 50 == 0:
        print(f"Epoch {epoch:04d} | Total: {total_loss.item():.4f}[{((1.0 - (total_loss.item() / LOSS_RANGE)) * 100):.4f}% perfection] "
              f"(Reach: {loss_reach.item():.4f}, Cos: {loss_cosine.item():.4f}, "
              f"Smth: {loss_smooth.item():.4f}, Sing: {loss_singular.item():.4f}, Step: {loss_step.item():.4f})")
