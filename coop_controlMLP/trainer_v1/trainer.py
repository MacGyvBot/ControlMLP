...
for epoch in range(EPOCHS):
    ...
    for step in range(MAX_STEPS):
        ...
        # 5) [Step-wise 로스 누적]
        # 거리 로스 (이미 도달해서 마스크가 0이 된 로봇은, 남은 스텝 내내 
        # 아주 작은 dist 값만 계속 누적하므로 엄청난 '칭찬(Loss 최소화)'을 받게 됨!)
        loss_fast_reach += (dist / MAX_WORKSPACE_DIST).mean()
    ...
    loss_cosine = (cos_A + cos_A_up).mean() / 4.0 # 최대 4.0이므로 정규화

    # 4. 로스 스케일링(Auto-Normalization) 및 합산
    loss_fast_reach = loss_fast_reach / MAX_STEPS
    loss_singular = loss_singular / MAX_STEPS
    loss_smooth = loss_smooth / SMOOTH_DENOM
    
    #TOTAL LOSS#
    total_loss = loss_fast_reach + loss_cosine + loss_smooth + loss_singular
    ...
    if epoch % 50 == 0:
        print(f"Epoch {epoch:04d} | Total: {total_loss.item():.4f} "
              f"(Reach: {loss_fast_reach.item():.4f}, Cos: {loss_cosine.item():.4f}, "
